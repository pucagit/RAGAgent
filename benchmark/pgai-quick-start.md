   ...
   Create the following `compose.yaml` in a new directory:
    ...
    services:
      ...
      ollama:
        image: ollama/ollama
        volumes:
          - C:\Users\hanoi\.ollama\models:~/.ollama/models
...
1. **Pull the required embedding models**
   - Docker: `docker compose exec -it pgai-ollama-1 ollama pull nomic-embed-text`

2. **Connect to the database in your local developer environment**

   - Docker: `docker compose exec -it db psql`
   - psql:  `psql postgres://postgres:postgres@localhost:5432/postgres`

3. **Enable pgai on your database**

    ```sql
    CREATE EXTENSION IF NOT EXISTS ai CASCADE;
    ```

4. **Create the `blog` table with the following schema**
    ```sql
    CREATE TABLE blog (
...
6. **Create a vectorizer for `blog`**
